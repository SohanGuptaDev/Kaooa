...
def intersection_point(point1, point2):
    ...
    x_4 = (b_line2 - b_line1) / (m_line1 - m_line2)
    y_intersection = m_line1 * x_4 + b_line1
    return x_4, y_intersection
...
def draw_circle(tur1, list1, vulture1, 
                occupied_array1, index1, 
...
def all_empty(occupied_array1, choice, choice1, vulture1):
    # print("imp")
    # print(choice)e
    """"empty choice"""
    # print(choice)
    for ele in choice:
        flag = 1
        if occupied_array1[ele-1] == 2:
            for ele1 in dictionary[f"{ele}"]["nn"]:
                # print(ele1)
                if ele1 in choice1:
                    if occupied_array1[ele1-1] == 2:
                        # print("sohangupta")
                        flag = 1
                        # return True
                    if occupied_array1[ele1-1] == 0:
                        # print("sohanmaupa")
                        flag = 0
                        return False
                        # return False
...
def in_circle(x_x, y_y, tur1, flag, vulture, crow, click_count,
    ...
    for i in range(len(intersected_list)):
        ...
        if ((m.sqrt((x_x-intersected_list[i][0])**2+(y_y-intersected_list[i][1])**2)) < 30.0
                and flag[0] == 1 and occupied_array[i] == 0):
            # print(i)
            # print(occupied_array[i],i);
            print(f"crow count:{crows_count[0]}")
            # print("aaaaa")
            if len(vulture) == 1:
                # print("aaaaaa")
                ...
                if i+1 in choice1:
                    ...
                    if occupied_array[to_search-1] == 2:
                        ...
                        draw_circle(tur1, intersected_list[:(i+1)], vulture
                                    , occupied_array, to_search,
                                    False, False, True)
                        flag[0] = 0
                        occupied_array[i] = 1
                ...
                click_count[0] += 1
                return True
        if ((m.sqrt((x_x-intersected_list[i][0])**2+(y_y-intersected_list[i][1])**2)) < 30.0 and
                flag[0] == 0 and occupied_array[i] == 0 and len(crow) <= crows_count[0] and crows_filled[0]==0):
            print("clive")
            crow.append(intersected_list[i])
            draw_circle(tur1, intersected_list[:(i+1)], vulture,
                        occupied_array, 0,False, True)
            flag[0] = 1
            click_count[0] += 1
            occupied_array[i] = 2
        ...
        if (m.sqrt((x_x-intersected_list[i][0])**2+(y_y-intersected_list[i][1])**2) < 30.0 and
            ...
            if len(selected) == 2:
                print("draw")
                # print(selected);
                choice4 = dictionary[f"{selected[0]}"]["nn"]
                # print(choice4);
                if selected[1] in choice4:
                    draw_circle(tur, intersected_list[:(i+1)], vulture,
                                occupied_array, 0,
                ...
                # flag1=0
                    ...
                    occupied_array[selected[0]-1]=0
                    occupied_array[selected[1]-1]=2
                    selected .pop(0)
                    selected.pop(-1)
                    index4 .pop(0)
                    flag[0] = 1
