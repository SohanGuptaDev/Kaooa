...
def draw_circle(tur, list1, vulture, crow, click_count, 
occupied_array, index1, kill_count, selected,
    ...
    for i in range(len(intersected_list)):
        ...
        if ((m.sqrt((x-intersected_list[i][0])**2+(y-intersected_list[i][1])**2))<30.0 and flag[0]==1 and occupied_array[i]==0):
            ...
            if len(vulture)==1:
                ...
                vulture.append(intersected_list[i])
                draw_circle(tur,intersected_list[:(i+1)],vulture,
                            crow,click_count,occupied_array,0,kill_count,selected,True,False)
                flag[0]=0
                occupied_array[i]=1
                click_count[0]+=1
                return True
        ...
        if (m.sqrt((x-intersected_list[i][0])**2+(y-intersected_list[i][1])**2)<30.0 and flag[0]==0 and crows_filled[0]==1):
            ...
            if len(selected)==2:
                ...
                if selected in choice:
                    draw_circle(tur,intersected_list[:(i+1)],vulture,crow,
                                click_count,occupied_array,0,kill_count,selected,False,False,False,True)
                # flag1=0
                    print(index4)
...
def on_screen_click(x,y):
    if in_circle(x,y,tur,flag,vulture,crow,click_count,
